 ...
 * where xi, vi, ai, and ji are the position, velocity, acceration, and jerk in dimension i.
 * 
 */
class TripleIntegrator : public ContinuousDynamics {
 public:
  ...
  VectorXd Evaluate(const VectorXd& x, const VectorXd&u, const float t) const override;
  void Jacobian(const VectorXd& x, const VectorXd& u, const float t, MatrixXd& jac) const override; 
